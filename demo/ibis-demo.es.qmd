...
con.sql("""
        SELECT
        "t0"."tconst",
        CAST("t0"."averageRating" AS VARCHAR) AS "average_rating",
...
```{python}
basics.join(ratings, "tconst").order_by(_.average_rating.desc()).filter(_.num_votes > 1e6)
```
...
```{python}
topfilms = basics.join(ratings, "tconst").order_by(_.average_rating.desc()).filter(_.num_votes > 1e6)
```
